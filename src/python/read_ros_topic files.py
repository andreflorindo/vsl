...
def read_path(robot_state_from_file, robot_state_from_file_velocity, robot_state_from_file_acceleration):
    ...
    for line in infile:
        ...
        if len(input) != 0:
            robot_state_from_file.ee_request.time.append(float(input[2]))
    ...
    for line in infile:
        input = re.findall(r"-?\ *[0-9]+\.?[0-9]*(?:[Ee]\ *-?\ *[0-9]+)?", line)
        if len(input) != 0:
            robot_state_from_file.joint_request.time.append(float(input[2]))
    ...
    for line in infile:
        input = re.findall(r"-?\ *[0-9]+\.?[0-9]*(?:[Ee]\ *-?\ *[0-9]+)?", line)
        if len(input) != 0:
            robot_state_from_file.joint_states.time.append(float(input[2]))
...
def adjust_time(time):
    ...
    for i in range(1, len(time)):
        time[i] = (time[i]-buffer_start)/10**9
...
def fill_derivative_class(robot_state, robot_state_velocity):
    ...
    # EE Request Velocity
    robot_state_velocity.ee_request.x = compute_derivative(
        robot_state.ee_request.time, robot_state.ee_request.x)
    robot_state_velocity.ee_request.y = compute_derivative(
        robot_state.ee_request.time, robot_state.ee_request.y)
    robot_state_velocity.ee_request.z = compute_derivative(
        robot_state.ee_request.time, robot_state.ee_request.z)
    robot_state_velocity.ee_request.rx = compute_derivative(
        robot_state.ee_request.time, robot_state.ee_request.rx)
    robot_state_velocity.ee_request.ry = compute_derivative(
        robot_state.ee_request.time, robot_state.ee_request.ry)
    robot_state_velocity.ee_request.rz = compute_derivative(
        robot_state.ee_request.time, robot_state.ee_request.rz)
    for i in range(0, len(robot_state.ee_request.time)):
        robot_state_velocity.ee_request.linear.append(math.sqrt(robot_state_velocity.ee_request.x[i]**2 +
                                                     robot_state_velocity.ee_request.y[i]**2+robot_state_velocity.ee_request.z[i]**2))
...
if __name__ == "__main__":
    ...
    #  robot_state_acceleration)
    plot_all_joint(robot_state_from_file, robot_state_from_file_velocity, robot_state_from_file_acceleration)
